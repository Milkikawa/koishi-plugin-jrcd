...
本项目从 `@yyyhf/koishi-plugin-jrcd@1.2.1` 重构而来，修复了以下问题：

- `bbcd` 不再用全局中间件截获消息，改用 Koishi 正式命令参数。
- 修复比较时短于对方 11 ～ 20 cm 的文案无法触发的判断错误。
- 移除了 `String.prototype.hashCode` 全局污染和调试日志输出。
- 身份键加入平台信息，避免不同平台相同 ID 结果碰撞。
